...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  // TODO: See instructions below
	...
	#pragma omp parallel
	#pragma omp for schedule(static,8) //collapse(6)
			...
			for(int P=0; P<k; P+=BLOCK_SIZE){
				for(int i=0; i<BLOCK_SIZE; i++){	
						...
						for(int p=0; p<BLOCK_SIZE; p++){
			    				...
			    				double A_IP = a[(I+i) + m*(P+p)];
			   		        	double B_PJ = b[(P+p) + k*(J+j)];
			    				double C_IJ = c[(I+i) + m*(J+j)];
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    //double flops = (NREPEATS*2*n*m*k*(1e-9))/time;
    //double bandwidth = (NREPEATS*(3*k*n*m + m*n)*(1e-9)*(sizeof(double)))/time;
    ...
    double flops = (NREPEATS*2*n*m*k*(1e-9))/time; // TODO: calculate from m, n, k, NREPEATS, time
    double bandwidth = (NREPEATS*2*m*n*((k/BLOCK_SIZE)+1))*(1e-9)*(sizeof(double))/time; // TODO: calculate from m, n, k, NREPEATS, time
